...
Zinc 是一个专注、小型的 Windows 终端，基于 Electron、React、xterm.js 与
`node-pty` 构建。它优先支持 PowerShell 7，并保留紧凑的核心体验：垂直标签栏、
Acrylic 风格窗口、实用终端设置，以及会尝试恢复最近一次 AI 对话的会话恢复。
...
- 字体、配色、不透明度、快捷键、状态栏、缩放、回滚行数与会话恢复设置。
- 剪贴板图片粘贴、可点击网页链接；新建或复制标签会尝试继承当前工作目录，
  后续在 shell 中切换目录可能无法反映。
- 按用户安装的 Windows NSIS 安装包、带 payload 校验的便携式自定义安装器，以及可选的
...
- Zinc 0.5.0 仅支持 Windows：Windows 10 或 Windows 11，x64。其他平台没有计划。
...
首次安装用它。同一 Release 中的 `Zinc-<version>-Installer.exe` 是覆盖安装、重装与
卸载用的修复工具，内含同一个 Setup，因此体积约为其两倍；它不是便携版 Zinc。
